...
class Aluno:
    ...
    def alterar(self):
        if not alunos:
            print("Não possui alunos para alterar.")
            return
        for i, pessoa in enumerate(alunos, start=1):
            print(f"{i}-{pessoa}")
        try:
            altera_pessoa = int(input("Digite o número da pessoa que deseja alterar: "))
            if altera_pessoa < 1 or altera_pessoa > len(alunos):
                ...
                while True:
                    ...
                    if pedido < 1 or pedido > 3:
                        ...
                        if pedido == 1:
                            ...
                            alunos[altera_pessoa - 1] = novo_nome
                            print("Alteração realizada com sucesso")
                        elif pedido == 2:
                            nova_matricula = str(input("Digite a nova matrícula: "))
                            matriculas[altera_pessoa - 1] = nova_matricula
                            print("Alteração realizada com sucesso")
                        elif pedido == 3:
                            break
        except ValueError:
        ...
        if menu_principal < 1 or menu_principal > 7:
            ...
            if menu_principal == 1:
                ...
                if aluno_alterar in alunos:
                    index = alunos.index(aluno_alterar)
                    Aluno(alunos[index], matriculas[index]).alterar()
                else:
                    print("Aluno não encontrado.")
